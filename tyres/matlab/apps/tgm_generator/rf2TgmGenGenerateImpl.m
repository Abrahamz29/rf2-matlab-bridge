function report = rf2TgmGenGenerateImpl(options)
...
if status ~= 0
    error("rf2TgmGenGenerateImpl:CommandFailed", "%s failed:\n%s", commandName, output);
end
...
if status ~= 0 && strlength(strtrim(output)) == 0
    error("rf2TgmGenGenerateImpl:CompareFailed", "Compare failed:\n%s", output);
end
report = jsondecode(output);
end
...
error("rf2TgmGenGenerateImpl:PythonNotFound", "Could not find a Python executable.");
end
