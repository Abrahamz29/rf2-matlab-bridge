...
def parse_scalar(value: str) -> Any:
    ...
    fraction_prefix = re.match(r"^([-+]?\d+(?:\.\d+)?)/(\d+(?:\.\d+)?)\s*(?:[A-Za-z/%\"'Â°]+)?$", value)
    if fraction_prefix:
        return float(fraction_prefix.group(1)) / float(fraction_prefix.group(2))
    numeric_prefix = re.match(r"^([-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?)\s*(?:[A-Za-z/%\"'Â°]+)?$", value)
    if numeric_prefix:
...
class RecursiveFormulaEvaluator(CachedFormulaEvaluator):
    ...
    def ref(self, sheet: str, address: str) -> Any:
        ...
        if sheet == "General" and address.upper().replace("$", "") == "I47" and cell and cell.display.startswith("Ã¢â€ Â"):
            col -= 1
            key = (sheet, row, col)
            cell = self.cells.get(key, cell)
        if cell is None:
