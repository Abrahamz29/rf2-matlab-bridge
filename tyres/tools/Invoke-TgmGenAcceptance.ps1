...
try {
    ...
    if ($RunMatlabSmoke) {
        ...
        $matlabBatch = "addpath('tyres/matlab/functions'); report = rf2TgmGeneratorSmoke; fid = fopen('$matlabReportEscaped','w'); fwrite(fid, jsonencode(report), 'char'); fclose(fid); assert(report.passed);"
