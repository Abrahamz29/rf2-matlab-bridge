[CmdletBinding()]
param(
    [string]$OdsPath = "tyres/downloads/studio397/TGM Gen V0.33 - GY F1 1975 Front.ods",
    ...
    [string]$MatlabReportPath = "tmp/tgm_generator_matlab_smoke_report.json",
    [string]$PythonExe = "",
    [string]$MatlabExe = "",
    [switch]$RunMatlabSmoke,
    [switch]$PrepareTTool,
...
try {
    ...
    $output = & $PythonExe "tyres/tools/test_tgm_gen_ods_acceptance.py" `
        --ods $OdsPath `
        --out-dir $OutDir `
        --mode $Mode `
        --json
    ...
    $uiOutput = & $PythonExe "tyres/tools/test_tgm_generator_ui_static.py" --json
    ...
    if ($RunMatlabSmoke) {
        ...
        $matlabReportParent = Split-Path -Parent $MatlabReportPath
        if ($matlabReportParent -and -not (Test-Path -LiteralPath $matlabReportParent)) {
            New-Item -ItemType Directory -Force -Path $matlabReportParent | Out-Null
        }
        ...
        $matlabBatch = "addpath('bridge/matlab'); addpath('tyres/matlab'); addpath('tracks/blacklake/matlab'); report = rf2TgmGeneratorSmoke; fid = fopen('$matlabReportEscaped','w'); fwrite(fid, jsonencode(report), 'char'); fclose(fid); assert(report.passed);"
