function report = rf2TgmAllKnownTyresSmoke(options)
%RF2TGMALLKNOWNTYRESSMOKE Validate all locally known cached TGM tyres.
arguments
    options.TgmRoot (1,1) string = fullfile("tools", "cache", "tyres", "tgm")
    options.ReportPath (1,1) string = fullfile("tmp", "tgm_all_known_tyres_smoke_report.json")
    options.RoundtripDir (1,1) string = fullfile("tmp", "tgm_all_known_tyres_roundtrip")
...
if ~report.passed
    error("rf2TgmAllKnownTyresSmoke:Failed", "Known tyre smoke failed: %s", jsonencode(report));
end
end
...
try
    ...
    rf2WriteTgm(model, roundtripPath, "StripGeneratedLookups", false);
...
if fid < 0
    error("rf2TgmAllKnownTyresSmoke:OpenFailed", "Could not open report: %s", path);
end
