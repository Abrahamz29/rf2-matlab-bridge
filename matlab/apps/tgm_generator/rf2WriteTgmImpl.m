function rf2WriteTgmImpl(model, outPath, options)
...
if ~isfield(model, "lines")
    error("rf2WriteTgmImpl:InvalidModel", "Model must come from rf2ReadTgmImpl and contain original lines.");
end
...
if fid < 0
    error("rf2WriteTgmImpl:OpenFailed", "Could not open output file: %s", outPath);
end
