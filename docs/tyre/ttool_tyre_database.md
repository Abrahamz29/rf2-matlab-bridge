...
```powershell
& "C:\Users\Victor\.platformio\penv\Scripts\python.exe" .\tyres\tools\build_tyre_database.py
```
...
Kopierte `.tgm`-Arbeitskopien liegen lokal unter `tyres\cache\tgm`.
...
```powershell
& "C:\Users\Victor\.platformio\penv\Scripts\python.exe" .\tyres\tools\build_tyre_database.py --include-workshop-packages
```
...
```powershell
& "C:\Users\Victor\.platformio\penv\Scripts\python.exe" .\tyres\tools\build_tyre_database.py `
  --extra-tgm .\tyres\input\tgm\G_9.2-20.0-13x10_Soft_Slick_1975.tgm
```

## MATLAB-Smoke fuer alle bekannten Reifen

Alle lokal bekannten Arbeitskopien unter `tyres\cache\tgm` koennen gegen
...
```powershell
.\tyres\tools\Test-TgmAllKnownTyres.ps1
```
