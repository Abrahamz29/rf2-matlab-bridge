...
`downloads/` paths. The reproducible project scripts are:

- `tracks/blacklake/tools/Install-BlackLakeExportToolchain.ps1`
- `tracks/blacklake/tools/Export-BlackLakeGmt.ps1`
- `tracks/blacklake/tools/blender_export_blacklake.py`
...
`GRID`, `ALTGRID`, and `TELEPORT` are moved to central BlackLake coordinates
above the flat surface. Pit/garage entries intentionally remain on the donor
AIW coordinates, because rFactor 2 couples them to matching pitlane waypoints
and crashes if those coordinates are moved independently. It also copies the
known Joesville TDF as `BlackLake.tdf`, because the current custom BlackLake
TDF still crashes the retail client before `DynMan::Init`.
...
```powershell
& "C:\Users\Victor\.platformio\penv\Scripts\python.exe" .\tracks\blacklake\tools\blacklake_builder.py --stage 250m
```

Install one generated stage into rFactor 2 ModDev after GMT export:

```powershell
.\tracks\blacklake\tools\Install-BlackLakeModDev.ps1 -Stage 250m
```
